...
public unsafe class JGObject : GHandle
{
    ...
    public bool InstanceOf(Env env, ClassHandle clazz) => env.Master->IsInstanceOf(Addr, !clazz);
    public ClassHandle GetClass(Env env) => new(env, env.Master->GetObjectClass(Addr));
    public GClassHandle GetGlobalClass(Env env)
    {
        var cls = env.Master->GetObjectClass(Addr);
        return new GClassHandle(env.NewGlobalRef(cls), cls);
    }
    public WeakJObject AsWeak(Env env) => new(env, Addr);
    public bool Equals(Env env, JGObject obj) => env.Master->IsSameObject(Addr, !obj);
